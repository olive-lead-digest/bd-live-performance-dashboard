...
import { BDDirectory } from '@/components/BDDirectory';
  ...
  if (isLoading || !data) {
    return (
      <div className="flex flex-col items-center justify-center h-full gap-4 relative z-10">
        ...
        <div className="text-white font-bold tracking-widest uppercase text-sm animate-pulse">Loading directory</div>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-4 sm:gap-6 pb-20 relative">
      ...
      <header className="mb-2 relative z-10">
        ...
        <p className="text-text-secondary text-sm mt-1 font-medium">
          Every BD by region and region head, with Zoom number and email.
        </p>
      </header>
      ...
      {!data.org?.bds && (
        <div className="flex flex-col items-center justify-center gap-4 relative z-10 text-center px-6 py-20">
          <Users className="w-10 h-10 text-brand-purple-400" />
          <div className="text-text-secondary text-sm">Directory data is loading or unavailable.</div>
        </div>
      )}
    </div>
  );
