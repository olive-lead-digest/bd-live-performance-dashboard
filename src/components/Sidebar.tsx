...
import { LayoutDashboard, Trophy, GitMerge, Map, Users, Bot, Settings, Filter, SplitSquareHorizontal, BarChart3 } from 'lucide-react';
...
export function Sidebar({ onOpenFilters, onOpenAI }: { onOpenFilters: () => void, onOpenAI: () => void }) {
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 z-40 w-16 hover:w-64 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] bg-panel/90 backdrop-blur-2xl border-r border-border-subtle flex flex-col group overflow-hidden shadow-2xl">
      <div className="flex h-16 shrink-0 items-center justify-center group-hover:justify-start group-hover:px-6 w-full relative">
        <div className="w-8 h-8 rounded-full border border-brand-purple-400 flex items-center justify-center shrink-0 bg-brand-purple-900/50 shadow-[0_0_15px_rgba(80,40,117,0.5)]">
          <span className="text-white font-bold text-xs tracking-tighter">O</span>
        </div>
      ...
      <div className="mt-auto mb-4 px-2 flex flex-col gap-2">
        ...
        <button 
          onClick={onOpenAI}
          className="flex items-center w-full h-11 rounded-xl relative group/item hover:bg-surface/50 transition-colors"
        >
          <div className="w-12 h-full flex items-center justify-center shrink-0">
            <Bot className="w-5 h-5 text-text-secondary group-hover/item:text-brand-pink-400 transition-colors" />
          </div>
          <span className="whitespace-nowrap text-sm font-medium text-text-secondary opacity-0 group-hover:opacity-100 transition-opacity duration-300 group-hover/item:text-white">
            Ask AI
          </span>
